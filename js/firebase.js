...
// FIX: نضبط الـ flag فوراً قبل أي شيء آخر — onAuthStateChanged يُطلق قبل getRedirectResult
if (localStorage.getItem('azem_ob_redirect') === '1') {
...
window.firebaseSignIn = async function() {
  ...
  try {
    ...
    // نضبط الـ flag دائماً — Firebase قد يُحوّل popup إلى redirect تلقائياً على Android
    localStorage.setItem('azem_settings_redirect', '1');
    if (isStandalone) {
      await signInWithRedirect(auth, provider);
    } else {
      const result = await signInWithPopup(auth, provider);
      // إذا وصلنا هنا: popup نجح فعلاً
...
window.obFirebaseGoogleSignIn = async function() {
  ...
  try {
    ...
    if (isStandalone) {
      ...
      const result = await signInWithPopup(auth, provider);
      // إذا وصلنا هنا: popup نجح فعلاً (لم يُحوَّل لـ redirect)
...
getRedirectResult(auth).then(async result => {
  if (result && result.user) {
    ...
    // FIX: لا يوجد redirect result — نُعيد الـ flag إذا كان مضبوطاً بدون سبب
  ...
  window._obGoogleJustSignedIn = false; // FIX: reset عند الخطأ أيضاً
  localStorage.removeItem('azem_ob_redirect');
  localStorage.removeItem('azem_settings_redirect');
});
  ...
  if (user) {
    ...
    if (!window._obGoogleJustSignedIn) {
      ...
      if (!hasData && !S.onboardingDone) {
        // لا نعيد الـ onboarding إذا كان مفتوحاً بالفعل
...
window.openSettingsSheet = function() {
  ...
  // FIX: window._fbUser أكثر موثوقية من auth.currentUser
